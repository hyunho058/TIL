...
where id = 'a' and password = 'a123';
 ...
 --사용자별 등록글수
select id,count(*) as "게시글 등록 수"
from board
group by id
order by id;

--월별
select substr(regdate,4,2) as "월",count(id) as "개시글 수"
from board
group by substr(regdate,4,2)
order by substr(regdate,4,2);
...
select dept.dname,avg(sal), count(*),min(sal),max(sal),count(mgr)
from emp, dept
